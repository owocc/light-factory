...
<template>
    <!-- 添加/修改表单 -->
    <USlideover v-bind="$attrs" v-model="visible" @close="handlerClose">
            ...
            <template #footer>
                <slot name="footer">
                    <div class="grid items-center grid-cols-2 gap-x-4">
                        <UButton :loading="props.loading" class="w-full" block size="lg" @click="handlerFormSubmit">
                            {{ props.submitText }}
                        </UButton>

                        <UButton class="w-full" block color="gray" size="lg" @click="handlerClose">
                            取消
                        </UButton>
                    </div>
                </slot>
